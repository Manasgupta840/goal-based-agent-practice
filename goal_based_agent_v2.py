...
def extract_application_info(text: str) -> str:
    ...
    if name_match and not application_info["name"]:   # ✅ only fill if empty
        application_info["name"] = name_match.group(1).title()
        updates.append("✅ Name saved.")
    if email_match and not application_info["email"]:  # ✅ only fill if empty
        application_info["email"] = email_match.group(0)
        updates.append("✅ Email saved.")
    if skills_match and not application_info["skills"]:  # ✅ only fill if empty
        application_info["skills"] = skills_match.group(1).strip()
        updates.append("✅ Skills saved.")

    if not updates:
        return "ℹ️ No new info found (I already have your details)."

    return " ".join(updates) + " Let me check what else I need."
